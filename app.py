...
with tab_fac:
    if not df.empty:
        ...
        try:
            # Función rápida para limpiar plata localmente y no depender de la pestaña de facturación
            def plata_radar(x):
                try:
            ...
            # Calculamos inteligente los próximos dos meses (cambia de año automático si es diciembre)
            h_radar = hoy.date()
            ...
            # Filtramos el Excel de turnos (df_t)
            df_rad = df_t[df_t['Cancelado'] == False].copy() if 'df_t' in locals() else pd.DataFrame()
            
            if not df_rad.empty:
                # Aseguramos formato de fecha
                df_rad['Fecha_DT'] = pd.to_datetime(df_rad['Fecha_DT'], errors='coerce')
                
                # Cálculos Mes 1 (Ej: Mayo)
                ...
                # Traemos el objetivo (o le ponemos 150 por defecto por si no lo encuentra)
                ...
                with c_rad1:
                    st.markdown(f'''
                        <div class="metric-card" style="border-left: 5px solid #00A8E8;">
                            <div class="metric-title">Proyección {nombres_m[m1_m]}</div>
                            <div class="metric-value-number" style="color:#00509E;">{panos_m1:.1f} paños</div>
                            <div style="font-size: 0.9em; color: gray;">Estimación: {formato_pesos(plata_m1) if 'formato_pesos' in globals() else f"$ {plata_m1:,.0f}"}</div>
                        </div>
                    ''', unsafe_allow_html=True)
                    prog_m1 = min(int((panos_m1 / obj_panos_radar) * 100) if obj_panos_radar > 0 else 0, 100)
                    st.progress(prog_m1)
                    st.caption(f"Reservado el **{prog_m1}%** del objetivo mensual de {obj_panos_radar} paños.")
                    
                # Tarjeta Mes 2
                with c_rad2:
                    st.markdown(f'''
                        <div class="metric-card" style="border-left: 5px solid #6c757d;">
                            <div class="metric-title">Proyección {nombres_m[m2_m]}</div>
                            <div class="metric-value-number" style="color:#495057;">{panos_m2:.1f} paños</div>
                            <div style="font-size: 0.9em; color: gray;">Estimación: {formato_pesos(plata_m2) if 'formato_pesos' in globals() else f"$ {plata_m2:,.0f}"}</div>
                        </div>
                    ''', unsafe_allow_html=True)
                    prog_m2 = min(int((panos_m2 / obj_panos_radar) * 100) if obj_panos_radar > 0 else 0, 100)
                    st.progress(prog_m2)
                    st.caption(f"Reservado el **{prog_m2}%** del objetivo mensual de {obj_panos_radar} paños.")
        except Exception as e:
            pass # Si hay un error silencioso con los datos, simplemente no muestra nada y no rompe la app
