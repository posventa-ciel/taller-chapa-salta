...
try:
    ...
    ID_PLANILLA = "1yVeTn7UJV5izBURIXFjROwnH1xD8L3vDpesPVZzy45c" # Planilla de SALTA
    planilla = gc.open_by_key(ID_PLANILLA)
    hoja = planilla.worksheet("TURNOS") # Asegurate de que exista esta pestaña si vas a usar el Turnero
...
# --- VARIABLES SALTA ---
GIDS = {"TALLER SALTA": "609774337"} # Solo un grupo principal
...
def obtener_datos_maestros():
    ...
    for n, gid in GIDS.items():
        try:
            ...
            # Estandarización de columnas (Simplificada)
            renames = {}
            for c in d.columns:
...
# ==========================================
# PESTAÑA 1: TALLER (KANBAN SIMPLIFICADO)
# ==========================================
with tab_taller:
    if not df.empty:
        ...
        for idx, fase in enumerate(orden_ideal):
            with cols_kanban[idx]:
                ...
                # Tolerancia a errores de tipeo en las fases
                df_fase = df_kanban[df_kanban['Fase_Taller'].str.contains(fase[:4], na=False, case=False)] if fase != "SIN ASIGNAR" else df_kanban[df_kanban['Fase_Taller'] == ""]
                
                if not df_fase.empty:
                    for _, row in df_fase.iterrows():
                        color_borde = "#6c757d" if fase == "⛔ DETENIDOS" else "#17a2b8"
                        st.markdown(f"""
        ...
        st.info("Cargando datos o no hay vehículos en proceso.")

# ==========================================
# PESTAÑA 2: FACTURACIÓN Y REPUESTOS
# ==========================================
with tab_fac:
    if not df.empty:
        st.subheader("Análisis de Facturación: Mano de Obra + Repuestos")
        
        # Filtramos los que están facturados o aprobados para el cálculo
